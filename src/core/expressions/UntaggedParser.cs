...
namespace Cdrcs.Expressions
{
    ...
    public class UntaggedParser<R> : IParser
    {
        static readonly ITransform skipStructTransform = new Transform(
            Base: baseParser => baseParser.Skip(Expression.Constant(CdrcsDataType.BT_STRUCT)));
        ...
        public UntaggedParser(RuntimeSchema schema, PayloadCdrcsedFactory bondedFactory)
        ...
        public UntaggedParser(Type type, PayloadCdrcsedFactory bondedFactory)
        ...
        UntaggedParser(UntaggedReader<R> reader, DeferredSkip deferredSkip, RuntimeSchema schema, PayloadCdrcsedFactory bondedFactory)
        {
            this.reader = reader;
            this.schema = schema;
        ...
        public bool IsCdrcsed { get { return schema.IsCdrcsed; } }
        ...
        public Expression Scalar(Expression valueType, CdrcsDataType expectedType, ValueHandler handler)
        {
            ...
            return handler(reader.Read((CdrcsDataType)(valueType as ConstantExpression).Value));
        }
        ...
        public Expression Cdrcsed(ValueHandler handler)
        {
            if (schema.IsCdrcsed)
            {
                return handler(reader.ReadMarshaledCdrcsed());
            }

            var newCdrcsed = bondedFactory(reader.Param, Expression.Constant(schema));
        ...
        {
            Debug.Assert(valueType is ConstantExpression);
            var dataType = (CdrcsDataType)(valueType as ConstantExpression).Value;
        ...
        {
            var ctor =
                typeof(CdrcsedVoid<>).MakeGenericType(reader.Type).GetConstructor(reader.Type, typeof(RuntimeSchema));
        ...
        Expression SkipList()
        {
            return Container(null, (valueParser, elementType, next, count, arraySegment) =>
            {
                ...
                var elementDataType = (CdrcsDataType)(elementType as ConstantExpression).Value;

                if (elementDataType == CdrcsDataType.BT_UINT8 || elementDataType == CdrcsDataType.BT_INT8)
