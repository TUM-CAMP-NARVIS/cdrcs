...
namespace Cdrcs.Expressions
{
    ...
    /// <summary>
    /// Creates expression of type <see cref="ICdrcsed{T}"/> given a object type and value.
    /// </summary>
    /// <param name="objectType">Type of object to be stored in <see cref="ICdrcsed"/></param>
    ...
    public delegate Expression ObjectCdrcsedFactory(Type objectType, Expression value);
    ...
    public class ObjectParser : IParser
    {
        ...
        readonly ObjectCdrcsedFactory bondedFactory;

        public ObjectParser(Type type)
            : this(type, null)
        {}

        public ObjectParser(Type type, ObjectCdrcsedFactory bondedFactory)
        {
        ...
        public bool IsCdrcsed { get { return schemaType.IsCdrcsed(); } }
        ...
        Expression Field(ITransform transform, Expression structVar, UInt16 id, ISchemaField schemaField, IField field)
        {
            ...
            var fieldType = Expression.Constant(fieldSchemaType.GetCdrcsDataType());
            ...
            // of Convert
            if (fieldSchemaType.IsCdrcsBlob())
            {
            ...
            if (fieldSchemaType.IsCdrcsStruct() || fieldSchemaType.IsCdrcsed() || schemaField.GetModifier() != Modifier.Optional)
            {
            ...
            else if (fieldSchemaType.IsCdrcsBlob())
            {
            ...
            /*else
            {
                ...
                else if (fieldSchemaType.IsCdrcsContainer())
                {
...
*/            cannotOmit = Expression.Constant(true);
        ...
        public Expression Container(CdrcsDataType? expectedType, ContainerHandler handler)
        {
            ...
            ContainerItemHandler itemHandler = (item, next, count) => handler(
                ...
                Expression.Constant(itemType.GetCdrcsDataType()),
                next,
                count,
                null);
        ...
        {
            Debug.Assert(schemaType.IsCdrcsMap());
            ...
            return EnumerableContainer((item, next, count) => handler(
                ...
                Expression.Constant(itemType.Key.GetCdrcsDataType()),
                Expression.Constant(itemType.Value.GetCdrcsDataType()),
                next,
                Expression.Empty(),
                count));
        }
        ...
        {
            Debug.Assert(expectedType == schemaType.GetCdrcsDataType());
            return handler(typeAlias.Convert(value, schemaType));
        }

        public Expression Cdrcsed(ValueHandler handler)
        {
            if (schemaType.IsCdrcsed())
            {
                return handler(value);
            }

            var newCdrcsed = bondedFactory(objectType, value);
            return handler(newCdrcsed);
        }
        ...
        public override bool Equals(object that)
        {
            ...
            return schemaType.IsCdrcsStruct() && schemaType == (that as ObjectParser).schemaType;
        }
        ...
        static Expression NewCdrcsed(Type objectType, Expression value)
        {
            var ctor = typeof(Cdrcsed<>).MakeGenericType(objectType).GetConstructor(objectType);
        ...
        {
            Debug.Assert(schemaType.IsCdrcsContainer());
        ...
        {
            Debug.Assert(schemaType.IsCdrcsNullable());
            ...
            var loop = handler(
                valueParser,
                Expression.Constant(valueType.GetCdrcsDataType()),
        ...
        {
            Debug.Assert(schemaType.IsCdrcsBlob());
            ...
            var loop = handler(
                ...
                Expression.Constant(CdrcsDataType.BT_INT8),
