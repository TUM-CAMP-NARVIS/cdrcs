...
namespace Cdrcs.Expressions
{
    ...
    public abstract class SerializerGenerator<R, W> : ISerializerGenerator<R, W>
    {
        ...
        protected Expression GenerateSerialize(Serialize serialize, IParser parser, ParameterExpression writer, bool inline)
        {
            ...
            if (inline)
            {
                ...
                if (parser.ReaderParam != parser.ReaderValue && parser.ReaderValue.Type.IsCdrcsStruct())
                {
    ...
    internal class SerializerTransform<R, W> : SerializerGenerator<R, W>
    {
        ...
        static readonly bool binaryWriter = typeof(IProtocolReader).IsAssignableFrom(typeof(W).GetAttribute<ReaderAttribute>().ReaderType);
        ...
        Expression GenerateSerialize(SerializeWithSchema serializeWithSchema, IParser parser, RuntimeSchema schema)
        {
            ...
            var inline = !typeof(IProtocolReader).IsAssignableFrom(parser.ReaderParam.Type);
        ...
        {
            var expectedValueType = schema.HasValue ? schema.TypeDef.element.id : (CdrcsDataType?)null;
                ...
                (valueParser, elementType, next, count, arraySegment) =>
                {
                    ...
                    if ((blob != null) || (arraySegment != null))
                    {
                        body = PrunedExpression.IfThenElse(
                            Expression.Equal(elementType, Expression.Constant(CdrcsDataType.BT_INT8)),
                        ...
                        {
                            body = PrunedExpression.IfThenElse(
                                Expression.Equal(elementType, Expression.Constant(CdrcsDataType.BT_UINT8)),
                                writer.WriteBytes(arraySegment ?? blob),
                                body);
                        }
                    }
        ...
        {
            var expectedValueType = schema.HasValue ? schema.TypeDef.element.id : (CdrcsDataType?)null;
            var expectedKeyType = schema.HasValue ? schema.TypeDef.key.id : (CdrcsDataType?)null;
        ...
        Expression Value(IParser parser, Expression valueType)
        {
            if (parser.IsCdrcsed)
            {
                return parser.Cdrcsed(value =>
                    writer.WriteCdrcsed(PrunedExpression.Convert(value, typeof(ICdrcsed))));
            }
            ...
            var switchCases = new List<DeferredSwitchCase>
            {
                PrunedExpression.SwitchCase(
                    ...
                    CdrcsDataType.BT_LIST,
                    CdrcsDataType.BT_SET),
                PrunedExpression.SwitchCase(
                    () => GenerateSerialize(Map, parser),
                    CdrcsDataType.BT_MAP),
                PrunedExpression.SwitchCase(
                    () => GenerateSerialize(Struct, parser),
                    CdrcsDataType.BT_STRUCT)
            };

            switchCases.AddRange(
                from type in new[]
                {
                    CdrcsDataType.BT_BOOL,
                    CdrcsDataType.BT_UINT8,
                    CdrcsDataType.BT_UINT16,
                    CdrcsDataType.BT_UINT32,
                    CdrcsDataType.BT_UINT64,
                    CdrcsDataType.BT_FLOAT,
                    CdrcsDataType.BT_DOUBLE,
                    CdrcsDataType.BT_STRING,
                    CdrcsDataType.BT_INT8,
                    CdrcsDataType.BT_INT16,
                    CdrcsDataType.BT_INT32,
                    CdrcsDataType.BT_INT64,
                    CdrcsDataType.BT_WSTRING,
                    CdrcsDataType.BT_ENUM,
                }
                select
                    PrunedExpression.SwitchCase(
        ...
        Expression Value(IParser parser, Expression valueType, RuntimeSchema schema)
        {
            ...
            if (parser.IsCdrcsed/* || (untaggedWriter && schema.IsCdrcsed)*/)
                return parser.Cdrcsed(value =>
                    writer.WriteCdrcsed(PrunedExpression.Convert(value, typeof(ICdrcsed))));
