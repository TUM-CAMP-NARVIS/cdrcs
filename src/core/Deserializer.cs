...
namespace Cdrcs
{
    ...
    public class Deserializer<R>
    {
        ...
        /// <returns><see cref="Cdrcs.Deserialize.Result"/> indicating if and how deserialization failed.</returns>
        public Deserialize.Result TryDeserialize<T>(R reader, out T dest)
        {
            try
            {
                dest = Deserialize<T>(reader);
                return Cdrcs.Deserialize.Result.Success;
            }
            catch (Exception ex) when (ex is InvalidDataException || ex is EndOfStreamException)
            {
                dest = default(T);
                return Cdrcs.Deserialize.Result.InvalidData;
            }
        }
    ...
    {
        /// <summary>
        /// Deserialize an object from an <see cref="ICdrcsed{T}"/> instance using a specific deserializer
        /// </summary>
        /// <typeparam name="R">Protocol reader</typeparam>
        /// <typeparam name="T">Type of source object in the bonded</typeparam>
        /// <param name="deserializer">Deserializer to be used to deserialize <see cref="ICdrcsed{T}"/> payload</param>
        /// <param name="bonded"><see cref="ICdrcsed{T}"/> instance representing payload</param>
        /// <remarks>Implemented as an extension method to avoid <see cref="ICloneable{R}"/> constraint on <see cref="Deserializer{R}"/></remarks>
        /// <returns>Deserialized object</returns>
        public static T Deserialize<T, R>(this Deserializer<R> deserializer, ICdrcsed<T> bonded)
            where R : ICloneable<R>
        {
            var b = bonded as Cdrcsed<T, R>;
            if (b == null)
                throw new InvalidOperationException(string.Format("Expected Cdrcsed<{0}, {1}>", typeof(T), typeof(R)));
