namespace UnitTest
{
    ...
    public class InterfaceTests
    {
        ...
        interface IFoo
        {
            [Cdrcs.Id(0), Cdrcs.Type(typeof(Cdrcs.Tag.nullable<List<string>>))]
        ...
        interface IBar<T>
        {
            [Cdrcs.Id(0), Cdrcs.Type(typeof(Cdrcs.Tag.nullable<IList<Cdrcs.Tag.classT>>))]
            IList<T> Foos { get; set; }

            [Cdrcs.Id(1), Default(null)]
            string Name { get; set; }

            [Cdrcs.Id(2), Default(null)]
        ...
        {
            [Cdrcs.Id(0), Cdrcs.Type(typeof(Cdrcs.Tag.bonded<Cdrcs.Tag.classT>))]
            T LazyT { get; set; }

            [Cdrcs.Id(1), Cdrcs.Type(typeof(Cdrcs.Tag.bonded<IFoo>))]
            ...
            where R : ICloneable<R>
        {
            ...
            protected Lazy(ICdrcsed bonded)
            {
                Debug.Assert(!(bonded is Cdrcsed<T>));
            ...
            where R : ICloneable<R>
        {
            ...
            public LazyFoo(ICdrcsed bonded)
                : base(bonded)
            {}
        ...
        static Expression Factory2<R>(Type type, Type schemaType, params Expression[] arguments) 
            where R : ICloneable<R>
        {
            if (schemaType == typeof(Cdrcs.Tag.bonded<IFoo>))
