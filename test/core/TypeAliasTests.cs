namespace UnitTest.Aliases
{
    ...
    public class Lazy<T> : ICdrcsed<T>
    {
        readonly ICdrcsed<T> bonded;
        T instance;

        public Lazy()
        {
            bonded = Cdrcsed<T>.Empty;
        }

        public Lazy(ICdrcsed bonded)
        {
        ...
        public Lazy(T instance)
        {
            ...
            this.bonded = new Cdrcsed<T>(instance);
        }
        ...
        {
            return Cdrcs.Comparer.Equal(Value, (obj as Lazy<T>).Value);
        }
    ...
    public class TypeAliasTests
    {
        ...
        [Test]
        public void AliasCdrcsed()
        {
            var from = new CdrcsedAlias {lazy = new Lazy<Foo>(UnitTest.Random.Init<Foo>())};
            Assert.IsTrue(Reflection.IsCdrcsed(typeof(Lazy<Foo>)));
            TestTypeAliases(from);
        }

        [Test]
        public void AliasGenericCdrcsed()
        {
            var from = new GenericCdrcsedAlias<Foo> { lazy = new Lazy<Foo>(UnitTest.Random.Init<Foo>()) };
            Assert.IsTrue(Reflection.IsCdrcsed(typeof(Lazy<Foo>)));
            TestTypeAliases(from);
        }
        ...
        // type is not part of Cdrcs.TypeDef
        [Test]
        public void AliasesListDataType()
        {
            ...
            {
                ListSubType fieldListSubType = field.GetSchemaType().GetCdrcsListDataType();
    ...
    {
        [Cdrcs.Id(0), Cdrcs.Type(typeof(bool))]
        public Box<bool> c_bool;

        [Cdrcs.Id(1), Cdrcs.Type(typeof(sbyte))]
        public Box<sbyte> c_int8;

        [Cdrcs.Id(2), Cdrcs.Type(typeof(short))]
        public Box<short> c_int16;

        [Cdrcs.Id(3), Cdrcs.Type(typeof(int))]
        public Box<int> c_int32;

        [Cdrcs.Id(4), Cdrcs.Type(typeof(long))]
        public Box<long> c_int64;

        [Cdrcs.Id(5), Cdrcs.Type(typeof(byte))]
        public Box<byte> c_uint8;

        [Cdrcs.Id(6), Cdrcs.Type(typeof(ushort))]
        public Box<ushort> c_uint16;

        [Cdrcs.Id(7), Cdrcs.Type(typeof(uint))]
        public Box<uint> c_uint32;

        [Cdrcs.Id(8), Cdrcs.Type(typeof(ulong))]
        public Box<ulong> c_uint64;

        [Cdrcs.Id(9), Cdrcs.Type(typeof(float))]
        public Box<float> c_float;

        [Cdrcs.Id(10), Cdrcs.Type(typeof(double))]
        public Box<double> c_double;

        [Cdrcs.Id(11), Cdrcs.Type(typeof(string))]
        public Box<string> c_string;

        [Cdrcs.Id(12), Cdrcs.Type(typeof(Cdrcs.Tag.wstring))]
